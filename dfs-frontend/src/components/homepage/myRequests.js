...
export default function MyRequests() {
  ...
  useEffect(() => {
    ...
    axios.get(url + 'requests-tome?token=' + JSON.parse(localStorage.getItem('dfs-user'))['token'])
    .then(res => {
  ...
  return (<>
    <div className="flex items-center justify-between pb-6">
      <div className="flex items-center justify-between">
        <p className="mt-2 text-3xl leading-8 font-bold tracking-tight text-gray-900 sm:text-4xl" style={{fontWeight: '500'}}>Requests to Me</p>
      </div>
    </div>
    <div>
      <div className="overflow-x-auto">
                ...
                <td className="px-3 py-3 border-b border-gray-200 bg-white text-sm">
                  {req.stat === 'requested' && <>
                    <button className="btn btn-outline-primary" onClick={e=>{
                      console.log(req);
                      axios.post(url + 'request-action?token=' + JSON.parse(localStorage.getItem('dfs-user'))['token'],
                      {...req, action: 'accepted'})
                      .then(res => {
                        console.log(res);
                        setRef(1-ref);
        ...
        </div> : <p>File Requests to you will appear here</p>}
      </div>
    </div>
  </>)
};
